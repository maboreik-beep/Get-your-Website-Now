...
const ContactSectionComponent: React.FC<{ section: ContactSection, themeColor: string, websiteTheme: 'light' | 'dark', website: Website; currentContentLanguage: string; path: string; }> = ({ section, themeColor, websiteTheme, website, currentContentLanguage, path }) => {
    ...
    const layoutClasses = {
        'address-left-form-right': 'grid md:grid-cols-2 gap-12 items-start',
        'form-left-address-right': 'grid md:grid-cols-2 gap-12 items-start md:grid-flow-col-dense', // Reverse order for form-left
    };

    const infoOrder = section.layout === 'address-left-form-right' ? 'md:order-1' : 'md:order-2';
    const formOrder = section.layout === 'address-left-form-right' ? 'md:order-2' : 'md:order-1';

    return (
        <SectionWrapper section={section} themeColor={themeColor} websiteTheme={websiteTheme}>
            <h2 className={`text-4xl lg:text-5xl font-bold text-center mb-16 ${titleColor}`}>{getTranslatedText(section.title, `${path}.title`, website, currentContentLanguage)}</h2>
            <div className={layoutClasses[section.layout || 'address-left-form-right']}>
...
const FooterSectionComponent: React.FC<{ section: FooterSection, themeColor: string, websiteTheme: 'light' | 'dark', website: Website; currentContentLanguage: string; path: string; }> = ({ section, themeColor, websiteTheme, website, currentContentLanguage, path }) => {
    ...
    const isDark = effectiveTheme === 'dark';
    const textColor = isDark ? 'text-gray-400' : 'text-gray-600';
    const linkColor = isDark ? 'text-dark-text-secondary' : 'text-gray-700';
    const bgColor = isDark ? 'bg-dark-bg' : 'bg-gray-100';

    return (
        <footer className={`py-12 px-4 md:px-8 ${bgColor}`} style={{ color: textColor }} role="contentinfo">
            <div className="container mx-auto max-w-7xl flex flex-col md:flex-row items-center justify-between gap-6">
                <Logo className="h-10 w-auto" />
                <p className="text-sm text-center md:text-left">{getTranslatedText(section.text, `${path}.text`, website, currentContentLanguage)}</p>
                <div className="flex gap-4 text-xl">
                    {section.socialLinks?.twitter && <a href={section.socialLinks.twitter} aria-label="Twitter" className={`hover:text-primary transition-colors ${linkColor}`}><Icon name="Twitter" /></a>}
                    {section.socialLinks?.linkedin && <a href={section.socialLinks.linkedin} aria-label="LinkedIn" className={`hover:text-primary transition-colors ${linkColor}`}><Icon name="Linkedin" /></a>}
                    {section.socialLinks?.facebook && <a href={section.socialLinks.facebook} aria-label="Facebook" className={`hover:text-primary transition-colors ${linkColor}`}><Icon name="Facebook" /></a>}
                    {section.socialLinks?.instagram && <a href={section.socialLinks.instagram} aria-label="Instagram" className={`hover:text-primary transition-colors ${linkColor}`}><Icon name="Instagram" /></a>}
                </div>
            </div>
        </footer>
    );
...
// Main WebsiteComponent
const WebsiteComponent: React.FC<{ activePage: Page; website: Website; }> = ({ activePage, website }) => {
    const { currentContentLanguage } = useContext(WebsiteContext)!;

    const enabledSections = useMemo(() => {
        return activePage.sections.filter(section => section.enabled);
    }, [activePage.sections]);

    return (
        <div className="min-h-screen">
            {enabledSections.map((section, index) => {
                // Calculate dynamic path for content translation lookup
                // Find the index of the activePage within website.pages
                const activePageIndex = website.pages.findIndex(p => p.id === activePage.id);
                // Find the index of the current section within activePage.sections
                const sectionIndex = activePage.sections.findIndex(s => s.id === section.id);
                const sectionPath = `pages.${activePageIndex}.sections.${sectionIndex}`;
                // console.log(`Rendering section ${section.type} with path: ${sectionPath}`);

                switch (section.type) {
                    case 'Hero':
                        return <HeroSectionComponent key={section.id} section={section} themeColor={website.themeColor} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'About':
                        return <AboutSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Story':
                        return <StorySectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Services':
                        return <ServicesSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Products':
                        return <ProductsSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Features':
                        return <FeaturesSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Pricing':
                        return <PricingSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Clients':
                        return <ClientsSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Team':
                        return <TeamSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Gallery':
                        return <GallerySectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Testimonials':
                        return <TestimonialsSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'FAQ':
                        return <FAQSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'CTA':
                        return <CTASectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Blog':
                        return <BlogSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Contact':
                        return <ContactSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    case 'Footer':
                        return <FooterSectionComponent key={section.id} section={section} themeColor={website.themeColor} websiteTheme={website.theme} website={website} currentContentLanguage={currentContentLanguage} path={sectionPath} />;
                    default:
...
export default WebsiteComponent; // Default export
